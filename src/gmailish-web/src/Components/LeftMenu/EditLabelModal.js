const EditLabelModal = ({ show, darkMode, value, onChange, onCancel, onSubmit }) => {
  if (!show) return null;
        ...
        <div className="modal-content" style={{ backgroundColor: theme.background, border: 'none' }}>
          ...
          <div className={`modal-body ${theme.text}`} style={{ border: 'none' }}>
            <input
              ...
              style={{ backgroundColor: darkMode ? '#555777' : '#fff' }}
            />
          </div>
          <div className={`modal-footer ${theme.text}`} style={{ border: 'none' }}>
            <button className="btn btn-secondary" onClick={onCancel}>Cancel</button>
            <button className={theme.primaryButton} onClick={onSubmit}>Update Label</button>
          </div>
        </div>
      </div>
    </div>
